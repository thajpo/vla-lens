...
CONTACT_SAMPLE_PHASE = "post_env_step"
CONTACT_SAMPLING_LIMITATION = (
    "One MuJoCo contact-manifold sample is read immediately after each env.step control "
    "step; this is not exhaustive physics-substep collision accounting."
)
...
def extract_contact_context(
    ...
    *,
    ...
    status.available("contact", "sample_phase", "capture runner post-env.step hook")
...
def contact_capability_audit(
    ...
    if not snapshots:
        return {
            **_base_capability(),
            ...
            "reason": "no post-env.step contact snapshots were supplied",
            "sample_count": 0,
            "contact_count": 0,
            "mujoco_version": "",
    ...
    if available:
        ...
        reason = "contact snapshots do not all use post_env_step semantics"
...
def _base_capability() -> dict[str, Any]:
    return {
        ...
        "sample_phase": CONTACT_SAMPLE_PHASE,
        "sample_phase_semantics": "immediately after env.step returns for action[timestep]",

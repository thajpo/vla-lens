...
1. The GitHub campaign issue is the short human control surface: current gate,
   budget, next action, and blocker.
...
3. The campaign event directory is append-only and hash-chained. It records
   locks, pool access, budget reservations, attempts, audits, results, and
   advancement without becoming a second protocol.
...
The program plan contains no mutable status. Lifecycle state belongs in its
GitHub issue and generated research-run records, so changing `planned` to
`running` cannot alter scientific identity. Each result-bearing study gets a
...
The child binds the exact program and study, predecessor results, family pool,
...
This verifies exact input and audit bytes, Git tracking and cleanliness, the
child's lock commit, and current free space. The PI0.5 wrapper must still run
its capture-runtime check at launch. A valid umbrella plan alone never
authorizes hardware execution.

## Append-Only Campaign History

Every state change gets one immutable event. Concurrent agents serialize
appends, and each event records the exact previous event-file hash. Editing an
old event breaks the chain.

```bash
uv run python scripts/research_campaign_event.py append \
  --program configs/campaigns/rq024_controlled_scene_to_behavior.yaml \
  --event-root /path/to/rq024/events \
...
The GitHub issue remains the short current view. The event chain lets another
agent reconstruct lock → attempts → audit → result → advance without trusting
hand-edited status text.
...
access log are committed before confirmation outputs exist. Confirmation
failure supersedes the exploratory positive.
...
The renderer binds actual program, child, trial, analysis, audit, artifact, and
predecessor hashes. It derives the verdict and next action from structured
checks and the program rather than trusting prose. Effect intervals state

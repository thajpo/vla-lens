...
| `test_intervention_record_distinguishes_preflight_noop_intervened_control` | Intervention result schema distinguishes status/trial kinds and control roles | `InterventionRunSpec`; `src/vla_lens/interventions/results.py`; workbench API | Saved run with original/no-op/intervention/control trials | UI treating inspected/preflight data as causal evidence. |
